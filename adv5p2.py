...
for update in updates:
    ...
    if not all(valid(pages[i], pages[i+1:], pages[:i]) for i in range(len(pages))):
        dg = nx.DiGraph()
        for p in pages:
            for b in before.get(p, []):
                dg.add_edge(p, b)

        new_pages = [p for p in nx.topological_sort(dg) if p in pages]
        sum += new_pages[len(new_pages) // 2]
print(sum)